...
window.onload = function () {
	...
	function documentActions(e) {
		...
		if (targetElement.classList.contains('panel__form-icon')) {
			document.querySelector('.panel__form').classList.toggle('_active');
		} else if (!targetElement.closest('.panel__form') && document.querySelector('.panel__form._active')) {
			document.querySelector('.panel__form').classList.remove('_active');
		}

		// Закрытие меню при клике на поиск
		$('.panel__form-icon').on("click", function () {
...
// Добавление класса сквайпера при определенном разрешении
$(window).resize(function () {
	if ($(window).width() <= 768) {
	} else {
		$('.quality__body').removeClass('_swiper');
	};
});
